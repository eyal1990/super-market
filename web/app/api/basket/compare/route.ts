...
export async function POST(request: Request) {
  ...
  try {
    ...
    const validation = validateBasketItemsDetailed(items, true, catalog.products);
    ...
    const totals = Object.fromEntries(catalogStores.map((store) => [store.id, { store, ...serializeBasketCalculation(safeItems, store.id, new Date(), catalog.products) }]));
    return NextResponse.json({ selectedStore, totals, catalog: catalog.completeness, catalogSource: catalog.source, fallbackUsed: catalog.fallbackUsed, warnings: catalog.warnings, directory: directory.completeness, note: 'מחירים עשויים להשתנות בקופה; המחיר בסניף הוא הקובע.' }, { headers: { 'cache-control': 'no-store' } });
  } catch {
