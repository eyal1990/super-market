...
export async function POST(request: Request) {
  ...
  try {
    ...
    const validation = validateBasketItemsDetailed(payload.items);
    ...
    const handoff = buildDeliveryHandoff(items, payload.storeId, new Date(), storesFromDirectory(stores, directory.entries));
    ...
    return NextResponse.json({ handoff, note: 'זהו ייצוא/העברה בלבד. האתר לא ביצע הזמנה ולא קיבל פרטי תשלום.' }, { headers: { 'cache-control': 'no-store' } });
  } catch {
