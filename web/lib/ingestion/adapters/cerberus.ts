...
export async function* parseCerberusStores(file: DownloadedSourceFile, contextArg?: ParseContext): AsyncIterable<NormalizedStore> {
  ...
  for (let index = 0; index < nodes.length; index += 1) {
    ...
    yield {
      ...
      chainName: field(node, ['chainname', 'chainlabel', 'retailername']),
      name,
      address: field(node, ['address', 'street', 'streetname']),
      city: field(node, ['city', 'town']),
      district: field(node, ['district', 'region', 'area']),
...
export async function* parseCerberusPrices(file: DownloadedSourceFile, contextArg?: ParseContext): AsyncIterable<NormalizedPrice> {
  ...
  for (let index = 0; index < nodes.length; index += 1) {
    ...
    yield {
      ...
      brand: field(node, ['brand', 'brandname', 'manufacturer']),
      size: field(node, ['size', 'packsize', 'packaging']),
      category: field(node, ['category', 'categoryname', 'department']),
      aliases: [...new Set(findXmlNodes(node, ['alias', 'alternateName', 'searchterm']).map(elementText).filter((value): value is string => Boolean(value)))],
      imageUrl: field(node, ['imageurl', 'image', 'pictureurl', 'thumbnail']),
      imageAlt: field(node, ['imagealt', 'alttext']),
      priceNis,
